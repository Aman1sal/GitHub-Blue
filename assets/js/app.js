...
window.updateProfile = function (profileUrl) {
  ...
  fetchData(profileUrl, data => {
    ...
    repoUrl = repos_url;
    followUrl = followers_url;
    followerUrl = following_url.replace("{/other_user}", "");
    ...
    $profileCard.innerHTML = `
      ...
      ${location ?
        `<li class="meta-item">
          ...
          <span class="meta-text" aria-hidden="true">${location}</span>
        </li>` : ""
      }
      ...
      ${company ?
        `<li class="meta-item">
          ...
          <span class="meta-text" aria-hidden="true">${company}</span>
        </li>` : ""
      }
        ...
        `<li class="meta-item">
          ...
          <a href="${website}" target="_blank" class="meta-text" aria-hidden="true">${website.replace("https://", "")}</a>
        </li>` : ""
      }
      ...
      <ul class="profile-stats">
        <li class="stats-item">
          <span class="body">${public_repos}</span> Repo
        </li>
        <li class="stats-item">
          <span class="body">${numberTokilo(followers)}</span> Followers
        </li>
        <li class="stats-item">
          <span class="body">${numberTokilo(following)}</span> Follwing
        </li>
      </ul>
...
const updateRepository = function () {
  fetchData(`${repoUrl}?q=postman&order=desc&per_page=12`, function (data) {
...
const /**{NodeElement} */ $forkpanel = document.querySelector("[data-fork-panel]");
...
const updateFollower = function () {
  ...
  fetchData(followUrl, function (data) {
    ...
    if (data.length) {
      for (const item of data) {
        ...
        const /**{NodeElement} */ $followercard = document.createElement("articel");
        $followercard.classList.add("card", "follower-card");
        $followercard.innerHTML = `
          <figure class="avatar-circle img-holder">
          <img src="${avatar_url}&s=64" alt="${username}" class="img-cover" width="56" height="56" loading="lazy">
          </figure>
...
fetchData(followerUrl, function(data){
  ...
  if (data.length) {
    for (const item of data) {
      ...
      const /**{NodeElement} */ $followingCard = document.createElement("articel");
